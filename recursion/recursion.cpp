...
//only ans;
vector<string> subSeq1(string str, int idx)
{
    if (idx == str.size())
    { // base case hit hote he empty list send kar do
        vector<string>base;
        base.push_back("");
        return base;
    }

    vector<string> res = subSeq1(str, idx + 1);
    vector<string> myans(res); // copy the last saved ans
    for (auto s : res)
    { // abhi jo ans aaya he usme add kar lo current idx
